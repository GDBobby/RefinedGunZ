...
void RequestCreateBot()
{
	...
	{
		ZChatOutput("Must be in game to create a brandon");
		return;
	}

	if (ZGetGameClient()->GetMatchStageSetting()->GetGameType() != MMATCH_GAMETYPE_TRAINING)
	{
		ZChatOutput("Must be in a training room to create a brandon");
		return;
	}
...
void RequestDestroyBot()
{
	if (!GetBotInfo().MyBot)
	{
		ZChatOutput("Your brandon hasn't been created");
		return;
	}
...
void UpdateToggleBotWidget()
{
	...
	{
		Widget->SetText(GetBotInfo().MyBot ? "Destroy brandon (&B)" : "Create brandon (&B)");
	}
}
